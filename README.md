...
2. Verify key possession with a challenge–response: encrypt a fresh, random challenge to an encryption-capable subkey of the key and send it over the verified channel; require the plaintext back. If you specifically need signing capability, ask for a signature over the challenge made by the signing subkey (see the notes below).
