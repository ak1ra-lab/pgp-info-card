...
1. 准备好卡片（本仓库），或者用 `gpg-key2ps` 打普通指纹纸条；自己的指纹要熟到能核对。
...
2. 用 challenge–response 验证对方持有私钥：生成一个新的随机挑战，加密到对方一个能加密的 subkey 上发过去，要求把明文发回来。如果更看重签名能力，也可以请对方用签名 subkey 对挑战签名（见下面的注意事项）。
...
- **私钥持有证明止步于 subkey。** 把挑战加密到加密 subkey，证明的是对方握有*那个 subkey* 的私钥，不是签发认证的主密钥。`caff` 就是这么做的，大家一般也接受；要求更高的话，可以请对方用主密钥重新签一次。签名 subkey 做的签名也只能证明那个 subkey，证明不了主密钥的私钥。
- **认证按 UID 分别签。** OpenPGP 认证由主密钥对单个 User ID 签发。只认证你确实核对过绑定关系的 UID——回信的邮箱、渠道有应答的化名、证件上的姓名。`caff` 每个 UID 单独发一封信，就是为这个。
- **指纹长度不一样。** 模板按 40 位十六进制字符的 v4 指纹（SHA-1）排版。v6 密钥（[RFC 9580](https://www.rfc-editor.org/rfc/rfc9580.html)）是 64 位（SHA-256），需要更宽或者分两行的布局；`--rfc4880bis` 草案里的实验性 v5 密钥长度一样。
